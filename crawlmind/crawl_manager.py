    ...
    async def crawl(self, url: str, max_depth: int) -> AggregatedCrawlResult:
        ...
        raw_json = json.dumps(
            [
                {
                    "url": r.url,
                    "structured_content": r.structured_content.to_dict(),
                }
                for r in results
            ],
            ensure_ascii=False,
        ...
        # 4) Rank keywords in that final structured content
        ranked_keywords = await self.ranker.rank(structured_content)
