...
- All previously visited URLs: JSON array of all previously visited URLs. Please note that you are not allowed to visit the same URL twice.
...
**INSTRUCTIONS:**
    ...
    - Please gather as much information as possible on the topic and provide a detailed report at the end of the process.
    - Only visit relevant URLs that are related to the subject of the domain BASE_DOMAIN. Do not wander off to irrelevant URLs or topics.
    - Try to visit as many relevant URLs as possible at once, as they would be visited in parallel. 
    - Try to limit search depth to 3-4 levels deep, unless you find something very interesting. We need to gather as much information as possible in a short time.
    - Stick to the task and do not deviate from the task at hand. Your job is to gather as much information as possible for the domain BASE_DOMAIN.
                        ...
                        function=LLMFunction(
                            ...
                            parameters=LLMFunctionParameters(
                                ...
                                properties={
                                    "links": {
                                        "type": "array",
                                        "description": "An array of absolute URLs to visit. These would be visited in parallel.",
                                        "items": {"type": "string"}
                                    },
                                    "additional_remarks": {
                                        "type": "string"
        ...
        for _ in range(max_depth):
            ...
            if current_action.name == ActionType.VISIT_URLS:
                ...
                if current_action.arguments:
                    ...
                    # links_to_visit = list(set(links_to_visit) - self.visited_urls)
